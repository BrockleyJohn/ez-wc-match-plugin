...
// We had an issue on one installation where require_once( ABSPATH . '/wp-config.php' );
// gave us a fatal error. The following is more robust.
...
require_once( plugin_dir_path(__FILE__) . 'class-easify-mf-web-service.php' );
require_once( plugin_dir_path(__FILE__) . 'class-easify-mf-shop.php' );
...
class Easify_WC_Plugin {
    ...
    public function receive_from_easify() {
        try {
            ...
            $ews = new Easify_MF_Web_Service(get_option('easify_username'), $this->easify_crypto->decrypt(get_option('easify_password')), null, get_option('easify_web_service_location'));
